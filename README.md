...
Serializable lint rules, diagnostics, safe fixes, reports, and fast rule execution for Frontier package catalogs, application graphs, manifests, test evidence, policies, workflows, workers, assets, traces, and source snippets.
...
```sh
npm install @shapeshift-labs/frontier-linter
```
...
if (!result.summary.valid) {
  console.log(formatLintSarif(result));
}
```

## Surface

- `lintFrontier(input, config)` runs deterministic built-in and custom rules over structural resources, edges, evidence, packages, and source snippets.
- `defineLintRule()` and `createLintRuleset()` expose ESLint-style rule authoring without tying the package to an AST parser.
- `formatLintJson()`, `formatLintJsonl()`, `formatLintSarif()`, and `formatLintGitHubAnnotations()` turn diagnostics into CI and code-scanning output.
- `applyLintFixes()` applies safe JSON Pointer fix operations to JSON-like documents.
...
`@shapeshift-labs/frontier-linter` depends only on `@shapeshift-labs/frontier`. It accepts manifest, application, policy, workflow, worker, asset, trace, test, benchmark, and package-boundary data through structural inputs instead of importing those packages. This keeps the linter usable in browsers, CI jobs, agents, standalone repos, and package release gates.

## CLI

```sh
frontier-lint frontier-graph.json --format sarif > frontier-lint.sarif
frontier-lint frontier-graph.json --format summary --fail-on warning
```

The CLI reads JSON arrays, Frontier lint input objects, or newline-delimited JSON. It supports `json`, `jsonl`, `sarif`, `github`, and `summary` output formats.

## Benchmarks

Frontier-only package measurements live in `benchmarks/package-bench.mjs`.
